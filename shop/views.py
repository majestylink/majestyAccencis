...
from .forms import CheckoutForm, CouponForm, RefundForm, PaymentForm, FashionRequestForm
from .models import Item, OrderItem, Order, Payment, Coupon, Refund, ProductCategory
from core.models import Address, UserProfile
...
class ItemDetailView(DetailView):
    template_name = "shop/product.html" #.format(themeVersion())
    model = Item
...
class CheckoutView(View):
    template_name = "shop/checkout.html" #.format(themeVersion())
        ...
        try:
            ...
            if form.is_valid():
                ...
                if payment_option == 'S':
                    ...
                    return redirect('shop:payment', payment_option='paystack')
                else:
                    messages.warning(
                ...
                messages.warning(self.request, "Please Check your Input")
                return redirect("shop:checkout")
        except ObjectDoesNotExist:
...
class PaymentView(View):
    template_name = "shop/payment.html" #.format(themeVersion())
    def get(self, *args, **kwargs):
        order = Order.objects.get(user=self.request.user.profile, ordered=False)
    ...
    def post(self, *args, **kwargs):
        ...
        order_uid = self.request.POST.get('order_uid') 
...
class OrderSummaryView(LoginRequiredMixin, View):
    template_name = "shop/order_summary.html" #.format(themeVersion())
    def get(self, *args, **kwargs):
        try:
            order = Order.objects.get(user=self.request.user.profile, ordered=False)
            context = {
...
def add_to_cart(request, slug):
    ...
    order_qs = Order.objects.filter(user=request.user.profile, ordered=False)
    if order_qs.exists():
        order = order_qs[0]
        ...
        order = Order.objects.create(
            user=request.user.profile, ordered_date=ordered_date)
        order.items.add(order_item)
        messages.info(request, "This item was added to your cart.")
        return redirect("shop:order-summary")
...
def get_coupon(request, code):
    try:
        ...
        messages.add_message(request, messages.ERROR, "This coupon does not exist", extra_tags='text-danger')
        ...
        if form.is_valid():
            try:
                ...
                order = Order.objects.get(
                    user=self.request.user.profile, ordered=False)
...
class RequestRefundView(View):
    template_name = "shop/request_refund.html" #.format(themeVersion())
    def get(self, *args, **kwargs):
        form = RefundForm()
        context = {
            'form': form
...
class RequestFashionView(LoginRequiredMixin, View):
    template_name = "shop/fashion_request.html" #.format(themeVersion())
    def get(self, *args, **kwargs):
        context = {}
        ...
        form = FashionRequestForm()
        context['form'] =  form
        
        return render(self.request, "shop/fashion_request.html", context)

    def post(self, *args, **kwargs):
        form = FashionRequestForm(self.request.POST)
        if form.is_valid():
            product_slug = self.request.GET.get('product')
            fashion = form.save(commit=False)
            fashion.user = self.request.user.profile
            if product_slug:
                # print('*'*20, product_slug)
                product = Item.objects.get(slug=product_slug)
                fashion.product = product
            
            fashion.save()
            messages.info(self.request, "Your request was received. You will be contacted shortly")
            next_url = self.request.META.get('HTTP_REFERER')
            # return redirect("shop:request-fashion")
            return redirect(next_url)

            # except Exception as e:
            #     print(e)
            #     messages.info(self.request, "An Error Occured, We have been Notified.")
            #     return redirect("shop:request-fashion")
        else:
            return redirect("shop:request-fashion")
